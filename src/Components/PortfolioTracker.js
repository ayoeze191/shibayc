  ...
  return (
    <div className="flex-1 w-full flex justify-center h-full font-poppins-Regular">
        <div className='text-white h-fit bg-Navy-Torea-Bay w-4/5 sm:w-fit py-10 sm:px-10 flex flex-col gap-2 px-4 box-border '>
            ...
            <h1 className=' text-3xl sm:text-5xl font-bold text-silver-sand flex flex-col text-center text-d'>Let Cashlno help<span>you</span> </h1>
            ...
            <div className='flex sm: bg-silver rounded-lg border-silver-sand border-4 flex-col sm:flex-row pr-0'><input type='text' placeholder='paste your wallet address' className='flex-1 py-4 rounded-l-lg text-black text-lg px-2'/><button className='px-4 text-center bg-d font-outfit-ExtraBold'>Go</button></div>
        </div>
    </div>
  )
}
